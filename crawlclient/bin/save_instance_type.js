...
(function (index, max, callback){
	if(index >= max){
		return callback(instancetype_array);
	}
	var rawcallee = arguments.callee;
	provider_meta[index].get_instancetype_list(function(instancetype_list){
		instancetype_array = instancetype_array.concat(instancetype_list);
		rawcallee(index+1, max, callback);
	});
})(0, provider_meta.length, save_instancetype);

function save_instancetype(instancetype_array){
	module.totle = instancetype_array.length;
	module.updated = 0;
	console.log("collected %d instance type totally!", module.totle);
	...
	mysqldb.initdb(function(err, conn){
		...
		for(var index in instancetype_array){
			...
			var str = util.format("insert into benchmark_instancetype values(default, '%s', '%s', %s, %s, %d, %s, '%s', '%s', '%s', '%s', %d);",
...
function insert_record(conn, querystring, retry, callback){
	...
	conn.query(querystring, function(err, result){
		...
		module.updated++;
		if(module.updated >= module.totle){
